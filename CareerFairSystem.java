...
public class CareerFairSystem implements PropertyChangeListener {
    ...
    CareerFair fair;
    private final Map<String, Candidate> candidatesByEmail = new HashMap<>();
    ...
    public FairPhase getCurrentFairPhase() {
        return fair.getCurrentPhase();
    }
    ...
    public List<Organization> getOrganizations() {
        if (fair.organizations == null) return Collections.emptyList();
        return new ArrayList<>(fair.organizations);
    }
    ...
    public Candidate registerCandidate(String displayName, String email,
        ...
        if (candidatesByEmail.containsKey(normalizedEmail)) {
            throw new IllegalStateException("Candidate email already exists: " + email);
        }
        ...
        candidate.requests = new ArrayList<>();
     ...
     */
    public Request createRequest(Candidate candidate, String desiredTags,
                          String preferredOrgs, int maxAppointments) {
        LocalDateTime now = SystemTimer.getInstance().getNow();
        if (!fair.canBook(now)) {
            throw new IllegalStateException("Requests are only allowed during BOOKINGS_OPEN.");
        }
        if (candidate == null) throw new IllegalArgumentException("Candidate is required.");
        if (desiredTags == null || desiredTags.isBlank()) {
            throw new IllegalArgumentException("At least one desired tag is required.");
        }
     ...
     */
    public Reservation manualBook(Candidate candidate, Offer offer, LocalDateTime start) {
        LocalDateTime now = SystemTimer.getInstance().getNow();
        if (!fair.canBook(now)) {
            throw new IllegalStateException("Manual booking is only allowed during BOOKINGS_OPEN.");
        }
        if (candidate == null) throw new IllegalArgumentException("Candidate is required.");
        if (offer == null) throw new IllegalArgumentException("Offer is required.");
        if (start == null || offer.startTime == null || !start.equals(offer.startTime)) {
        ...
        reservation.scheduledStart = offer.startTime;
        reservation.scheduledEnd   = offer.endTime;
        reservation.state          = ReservationState.CONFIRMED;

        if (candidate.reservations == null) candidate.reservations = new ArrayList<>();
        candidate.reservations.add(reservation);
        if (offer.reservations == null) offer.reservations = new ArrayList<>();
     ...
     */
    public void joinSession(Candidate candidate, String reservationId) {
        if (candidate == null) throw new IllegalArgumentException("Candidate is required.");
        ...
        LocalDateTime now = SystemTimer.getInstance().getNow();
        if (!fair.isLive(now)) {
            throw new IllegalStateException("Cannot join session before FAIR_LIVE.");
        }
        if (candidate.reservations == null) {
        ...
        for (Reservation reservation : candidate.reservations) {
            if (reservation != null && reservation.offer != null
                && reservationId.equals(reservation.offer.title)) {
                target = reservation;
                break;
            }
        }
        ...
        if (target.scheduledStart != null && now.isBefore(target.scheduledStart)) {
            throw new IllegalStateException("Too early to join this session.");
        }

        target.state = ReservationState.IN_PROGRESS;
    }

    public Reservation autoBookPublic(Candidate candidate, Request request) {
        return autoBook(candidate, request);
    }
}
