...
public class VirtualRoom {
	...
	public void enter(Candidate candidate) {
		...
		if (state != RoomState.OCCUPIED) {
			Logger.log(LogLevel.WARNING, "Attempted to enter room that is not occupied");
			return;
		}
	...
	/**
	 * Add an occupant to the room (alias for enter).
	 * @param candidate The candidate entering
	 */
	public void addOccupant(Candidate candidate) {
		if (candidate == null) {
			throw new IllegalArgumentException("Candidate cannot be null");
		}
		occupants.add(candidate);
		Logger.log(LogLevel.INFO, "Occupant added: " + candidate.getDisplayName());
	}

	/**
	 * Remove an occupant from the room (alias for leave).
	 * @param candidate The candidate leaving
	 */
	public void removeOccupant(Candidate candidate) {
		if (candidate == null) {
			throw new IllegalArgumentException("Candidate cannot be null");
		}
		occupants.remove(candidate);
		Logger.log(LogLevel.INFO, "Occupant removed: " + candidate.getDisplayName());
	}

	/**
	 * Add a meeting session to this room.
	...
	public void addSession(MeetingSession session) {
		...
		Logger.log(LogLevel.INFO, "Session added to VirtualRoom");
	}

	/**
	 * Remove a meeting session from this room.
	 * @param session The session to remove
	 */
	public void removeSession(MeetingSession session) {
		if (session == null) {
			throw new IllegalArgumentException("Session cannot be null");
		}
		sessions.remove(session);
		Logger.log(LogLevel.INFO, "Session removed from VirtualRoom");
	}
