...
public class Lobby {
	...
	/**
	 * Get the waiting queue (mutable for direct manipulation if needed).
	 */
	...
	/**
	 * Get the candidates (alias for getWaitingQueue).
	 */
	...
	public String listWaiting() {
		...
		int size = waitingQueue.size();  // Cache size to avoid repeated calls
		for (int i = 0; i < size; i++) {
			Candidate c = waitingQueue.get(i);
			if (c != null) {  // Null-safe check
