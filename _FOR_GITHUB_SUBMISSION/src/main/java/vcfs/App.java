    ...
    public static void main(String[] args) {
        ...
        try {
            SwingUtilities.invokeLater(() -> {
                ...
                Logger.info("═══════════════════════════════════════════════════════════════════════════════════");
                Logger.info("🎬 DEMO WORKFLOW STEP-BY-STEP (20 MINUTES FOR 100-PERSON AUDIENCE):");
                Logger.info("═══════════════════════════════════════════════════════════════════════════════════");
                Logger.info("");
                Logger.info("📋 PHASE 1: ADMIN SETUP (2-3 minutes) ---- Select 'Admin' Portal");
                Logger.info("  STEP 1: Create Organizations");
                Logger.info("    → Go to Management tab");
                Logger.info("    → Create 3 organizations (Apple, Google, Microsoft)");
                Logger.info("    → See them appear in Organizations table");
                Logger.info("  STEP 2: Add Booths to Each Organization");  
                Logger.info("    → For each org, create 2 booths (e.g., 'Senior Dev', 'Data Scientist')");
                Logger.info("    → Watch tables update in real-time");
                Logger.info("  STEP 3: Assign Recruiters to Booths");
                Logger.info("    → Assign 3 recruiters (Alice@google, Bob@microsoft, Carol@apple)");
                Logger.info("    → See Recruiters table populate");
                Logger.info("  STEP 4: Configure Timeline to START THE FAIR");
                Logger.info("    → Set Fair Open Date (today)");
                Logger.info("    → Set times (9:00 AM - 5:00 PM)");
                Logger.info("    → Click 'Set Timeline' ← THIS STARTS THE RECRUITING FAIR");
                Logger.info("  STEP 5: Monitor the Audit Log");
                Logger.info("    → Watch console at bottom: every action logs as 'Organization Created', etc.");
                Logger.info("");
                Logger.info("📋 PHASE 2: RECRUITER ACTIONS (2-3 minutes) ---- Select 'Recruiter' Portal");
                Logger.info("  ACTION 1: Publish Interview Offers");
                Logger.info("    → Go to 'Publish Offer' tab");
                Logger.info("    → Create 2-3 offers (Engineer, Data Scientist, DevOps)");
                Logger.info("    → Check Admin Console 'Offers' table - see offers appear INSTANTLY");
                Logger.info("  ACTION 2: Set Your Available Interview Times");
                Logger.info("    → Go to 'Manage Schedule' tab");
                Logger.info("    → Add 3-4 available time slots (9:00, 10:00, 11:00, 2:00 PM)");
                Logger.info("    → Candidates will book these slots");
                Logger.info("  IMPORTANT: After EACH action, show Admin Console updating in real-time");
                Logger.info("");
                Logger.info("📋 PHASE 3: CANDIDATE INTERACTIONS (3-4 minutes) ---- Select 'Candidate' Portal");
                Logger.info("  ACTION 1: Browse Available Offers");
                Logger.info("    → Go to 'Browse Offers' tab");
                Logger.info("    → See all offers published by recruiters");
                Logger.info("    → Enter interest tags (e.g., 'Software')'");
                Logger.info("  ACTION 2: Auto-Book Best Matches");
                Logger.info("    → Click 'Auto-Book Best Matches'");
                Logger.info("    → System books you into recruiter's available slot");
                Logger.info("    → See confirmation dialog");
                Logger.info("  ACTION 3: Go to Admin Console - Show the Booking");
                Logger.info("    → Check 'Bookings' table - see your new booking!");
                Logger.info("    → Check Audit Log - see 'Booking Created' event with timestamp");
                Logger.info("    → THIS PROVES REAL-TIME SYNC across all 3 portals");
                Logger.info("");
                Logger.info("📋 PHASE 4: DEMONSTRATE CROSS-PORTAL SYNCHRONIZATION (2 minutes)");
                Logger.info("  YOUR GOAL: Show that ALL 3 portals see changes INSTANTLY");
                Logger.info("  REPEAT this cycle 2-3 times (each ~30 seconds):");
                Logger.info("    1. Switch to ADMIN → Create new booth");
                Logger.info("    2. Check RECRUITER portal → New booth appears in dropdown");
                Logger.info("    3. Switch back to ADMIN → See change in Audit Log");
                Logger.info("");
                Logger.info("📋 PHASE 5: SHOW SYSTEM ARCHITECTURE (3-4 minutes)");
                Logger.info("  EXPLAIN (while demo is running):");
                Logger.info("    • Observer pattern: PropertyChangeListener shows updates");
                Logger.info("    • MVC separation: Controllers handle business logic");
                Logger.info("    • Thread-safe collections: ConcurrentHashMap for concurrent access");
                Logger.info("    • Auto-save: Data persisted every 5 seconds (DataPersistenceManager)");
                Logger.info("    • Session tracking: Every action logged by SessionManager");
                Logger.info("    • System state: Operations tracked by SystemStateManager");
                Logger.info("");
                Logger.info("📋 PHASE 6: CONCLUDE (1 minute)");
                Logger.info("  SUMMARY POINTS:");
                Logger.info("    ✓ 3 independent portals working seamlessly together");
                Logger.info("    ✓ Real-time data sync via PropertyChangeListener (Observer)");
                Logger.info("    ✓ All data auto-saved (crash recovery enabled)");
                Logger.info("    ✓ Full audit trail of all operations");
                Logger.info("    ✓ Professional UI with workflow guides embedded");
                Logger.info("");
                Logger.info("═══════════════════════════════════════════════════════════════════════════════════");
                Logger.info("💡 QUICK TIPS FOR PRESENTER:");
                Logger.info("═══════════════════════════════════════════════════════════════════════════════════");
                Logger.info("  • Each portal has DETAILED WORKFLOW INSTRUCTIONS on its dashboard");
                Logger.info("  • No notes needed - ALL instructions visible on-screen during demo");
                Logger.info("  • Use SystemTimer (bottom right) to advance time through the fair");
                Logger.info("  • Admin Console Audit Log shows EVERYTHING happening in system");
                Logger.info("  • Data is auto-saved - safe to close and reopen without data loss");
                Logger.info("  • Each portal's 'Portal Flow Guide' (? button) explains that portal");
                Logger.info("");
                Logger.info("📱 UI ENHANCEMENTS FOR DEMO-READINESS:");
                Logger.info("  ✓ All portals use professional dialogs (UIEnhancementUtils)");
                Logger.info("  ✓ Workflow guidance panels with emoji indicators on every screen");
                Logger.info("  ✓ Status bars showing Fair Status, Bookings, System health");
                Logger.info("  ✓ Real-time activity tracking in SessionManager");
                Logger.info("  ✓ Operation recording in SystemStateManager");
